...
public class EnemyAvancedLogic : MonoBehaviour
{
    public int routine;
    ...
    {
        if (Vector3.Distance(transform.position, target.transform.position) > 5 && !fight)
        {
            agent.enabled = false;
        ...
        else
        {
            ...
            else if (Vector3.Distance(transform.position, target.transform.position) > 0.1)
            {
                fight = true;
                agent.enabled = false;
                agent.speed = 0;
